...
pub mod kdf {
    ...
    pub fn derive_key_128(salt: &[u8], ikm: &[u8], info: &[u8]) -> [u8; 16] {
        let mut okm = [0u8; 16];
    ...
    pub fn derive_key_256(salt: &[u8], ikm: &[u8], info: &[u8]) -> [u8; 32] {
        let mut okm = [0u8; 32];
        let hkdf = Hkdf::<Sha256>::new(Some(salt), ikm);
        hkdf.expand(info, &mut okm).unwrap();
        okm
...
mod tests {
    ...
    fn make_derive_key_128(salt: &[u8], ikm: &[u8]) -> [u8; 16] {
        let info = "tag".as_bytes();
        kdf::derive_key_128(&salt, &ikm, &info)
    }
    
    fn make_derive_key_256(salt: &[u8], ikm: &[u8]) -> [u8; 32] {
        let info = "tag".as_bytes();
        kdf::derive_key_256(&salt, &ikm, &info)
    }
    ...
    fn test_kdf_128_len() {
        ...
        let key = make_derive_key_128(&salt, &ikm);
    ...
    #[test]
    fn test_kdf_128_determinism() {
        let salt = rand::random::<[u8; 128]>();
        let ikm = rand::random::<[u8; 255]>();
        let key1 = make_derive_key_128(&salt, &ikm);
        let key2 = make_derive_key_128(&salt, &ikm);
        assert_eq!(key1, key2);
    }
